...
//  WaterBottle
//
//  Created by Yulu Zhang on 2/27/19.
...
class DistributionController: UIViewController {
    
    let titleLabel: UILabel = {
        let title = UILabel()
        let titleAttributeTitle = NSMutableAttributedString(string: "Updated Water Intake Distribution",
    ...
    lazy var titleLabelCups: UILabel = {
        ...
        let currentDayKey2 = DistributionController.getCurrentDay() + ":day"
    ...
    let enterButton: UIButton = {
        ...
        let enterAttributeTitle = NSMutableAttributedString(string: "Wanna Correct the Intake?",
                                                            attributes: [.foregroundColor: UIColor.white,
                                                                         .font: UIFont.systemFont(ofSize: 22)])
        enter.setAttributedTitle(enterAttributeTitle, for: .normal)
        enter.backgroundColor = CONFIRM
        enter.layer.cornerRadius = 15
        enter.addTarget(self, action: #selector(undo_page), for: .touchUpInside)
        return enter
    }()
    ...
    let pieChart: PieChartView = {
        ...
        var getArr = UserDefaults.standard.array(forKey: DistributionController.getCurrentDay()) as? [Double]
        var getLabel = UserDefaults.standard.array(forKey: DistributionController.getCurrentDay() + ":label") as? [String]
    ...
    override func viewDidLoad() {
        ...
        let newDayKey = DistributionController.getCurrentDay()
        ...
        let newDayKey1 = DistributionController.getCurrentDay() + ":label"
    ...
    fileprivate func titleLabelCupsComponent() {
        ...
        titleLabelCups.snp.makeConstraints { (make) -> Void in
            ...
            make.left.equalTo(titleLabel.snp.left).offset(105)
        }
    }
    ...
    let confirmButton: UIButton = {
        ...
        let confirmAttributeTitle = NSMutableAttributedString(string: "Confirm My Intake",
                                                            attributes: [.foregroundColor: UIColor.white,
                                                                         .font: UIFont.systemFont(ofSize: 22)])
        confirm.setAttributedTitle(confirmAttributeTitle, for: .normal)
        confirm.backgroundColor = CONFIRM
        confirm.layer.cornerRadius = 15
        //if cancel the distribution would update, no link action is needed
        confirm.addTarget(self, action: #selector(home_page), for: .touchUpInside)
        return confirm
    }()
    
    fileprivate func confirmButtonComponent() {
        view.addSubview(confirmButton)
        
        confirmButton.snp.makeConstraints { (make) -> Void in
            make.height.equalTo(50)
            make.centerX.equalTo(enterButton)
            make.left.equalTo(enterButton)
            make.centerY.equalTo(enterButton.snp.bottom).offset(40)
//
//            make.left.equalTo(self.view.snp.left).offset(35)
//            make.right.equalTo(self.view.snp.right).offset(-35)
//            make.centerX.equalTo(self.view.snp.centerX)
//            make.centerY.equalTo(self.view.snp.centerY).offset(190)
    ...
    var dayArr = 0.0
    @objc func home_page() {
        let currentDayKey = DistributionController.getCurrentDay()
        ...
        // label
        let currentDayKey1 = DistributionController.getCurrentDay() + ":label"
        ...
        if UserDefaults.standard.object(forKey: currentDayKey1) != nil {
            ...
            labelArr.append(DistributionController.getCurrentTime())
